...
> Yefeng Yang, Coralie Williams, Kyle Morrison, Lorenzo Ricolfi, Jinming
> Pan, Malgorzata Lagisz, Shinichi Nakagawa. Bivariate multilevel
> meta-analyses of log response ratio and standardized mean difference
> towards robust and reproducible evidence syntheses in environmental
> and biological sciences. 2024 <!--# please add Alistair  -->
...
`metafor`, `clubSandwich`, `tidyverse`, `here`, `DT`, `readxl`,
`stringr`, `ggplot2`, `pander`.
...
<!--# we need to show to how to get I2 and other hetro measures using orchaRd??? -->
...
<!--# I wondered AIC stuff may be usefu to add to here in the table s4 and s5 discuss -->
...
<!--# adding some visualizations - we could galaxy plots discuss -->
...
output:
<!--# I wondered whether we should put some tips when the complex models do not run - how to get it running with http://www.metafor-project.org/doku.php/tips:convergence_problems_rma_mv -->
...
@pustejovsky2022meta) for small-sample size adjustment. Using the fitted
BMLMA (the object `BMLMA`) as an example, the implementation of robust
variance estimation is as:
<!--# it will be nice to have robust for mulitlevel ones so you can fit this so people get exposed to the idea that robust can take any meta-analytic models -->

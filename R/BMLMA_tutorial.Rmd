...
author: "Yefeng Yang, Coralie Williams, Kyle Morrison, Lorenzo Ricolfi, Jinming Pan, Malgorzata Lagisz, Shinichi Nakagawa"
...
We will update this tutorial when necessary. Readers can access the latest version in our [GitHub repository](https://github.com/Yefeng0920/BRMA_tutorial_git).
...
`metafor`, `clubSandwich`, `tidyverse`, `here`, `DT`, `readxl`, `stringr`, `ggplot2`, `pander`.
...
You can find more details about the variables in the [paper](https://doi.org/10.1111/gcb.14646) by @burgess2021classifying, if interested. 
...
We can use the function `escalc()` in `metafor` package @viechtbauer2010conducting to compute the point estimate of log response ratio ($lnRR$) and associated sampling variance ($s_{lnRR}^2$) with:
...
Let's have a loot at the effect size estimates $SMD$ and their sampling variances $s_{SMD}^2$.
...
`tval` = the value of test statistic (in our case: t-value); a t-distribution with *k*−*p* degrees of freedom to test the overall effect and compute (95% by default) confidence intervals, 
...
__Accounting for the correlated errors in the MLMA:__
...
In the above syntex, setting `rho = 0.5` means we assume that sampling errors within the same study are correlated with $\rho$ = 0.5. 
...
__Table S4__
Sensitivity analysis examining the robustness of the overall effect measured as $lnRR$ to the assumption of $\rho_{s}$ values in the multilevel meta-analysis.  
...
colnames(t4) <- c("Sampling correlation", "Overall effect (lnRR)", "Standard error", "p-value", "Lower CI", "Upper CI")

t4 %>% dfround(4) %>% DT::datatable()
```

__Table S5__
Sensitivity analysis examining the robustness of the overall effect measured as $SMD$ to the assumption of $\rho_{s}$ values in the multilevel meta-analysis.  
...
colnames(t5) <- c("Sampling correlation", "Overall effect (SMD)", "Standard error", "p-value", "Lower CI", "Upper CI")
...
__Table S6__
Sensitivity analysis examining the robustness of the overall effect measured as $lnRR$ to the assumption of $\rho_{s}$ values in the bivariate multilevel meta-analysis.  
...
colnames(t6) <- c("Sampling correlation", "Overall effect (lnRR)", "Standard error", "p-value", "Lower CI", "Upper CI")

t6 %>% dfround(4) %>% DT::datatable()
```

__Table S7__
Sensitivity analysis examining the robustness of the overall effect measured as $SMD$ to the assumption of $\rho_{s}$ values in the bivariate multilevel meta-analysis.  
...
colnames(t7) <- c("Sampling correlation", "Overall effect (SMD)", "Standard error", "p-value", "Lower CI", "Upper CI")
